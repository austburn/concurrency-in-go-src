...
func PackItems(done <-chan bool, items <-chan Item, workerID int, printColor func(string, ...interface{})) <-chan int {
	...
	go func() {
		for item := range items {
			...
			case packages <- item.ID:
				...
				printColor("%d: Shipping package no. %d, took %ds to pack", workerID, item.ID, item.PackingEffort)
			}
		}
		close(packages)
	}()
...
func main() {
	...
	workerColors := []func(string, ...interface{}){
		color.Red,
		color.Green,
		color.Blue,
		color.Yellow,
	...
	for i, w := range workerColors {
		workers[i] = PackItems(done, items, i, w)
	}
	...
	color.Green("Took %fs to ship %d packages", time.Since(start).Seconds(), numPackages)
}
